...
export async function signUp(username: string): Promise<User> {
    ...
    if (userExists) throw new Error("Username already used on server");
  
    // should be handled in server - end
	//const userExists2 = !!db.getByUsername(username);
	//if (userExists2) throw new Error("Username already used");
